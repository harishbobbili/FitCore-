...
export default React.memo(function ExerciseCard({
  ...
  // Memoize exercise type checks to prevent recalculation on every render
  const isPlank = useMemo(() => exerciseName.toLowerCase().includes("plank"), [exerciseName]);
  const isPullUp = useMemo(() => exerciseName.toLowerCase().includes("pull-up"), [exerciseName]);
