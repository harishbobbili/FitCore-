...
export const WeeklyProgressChart = React.memo(() => {
  ...
  const weeklyData = useMemo(() => {
    if (!weeklyAnalytics?.daily_history) return [];
    return weeklyAnalytics.daily_history.map((log: any) => ({
  ...
  const weeklyConsistencyScore = useMemo(() => {
    return weeklyAnalytics?.summary?.consistency_score ?? 0;
  }, [weeklyAnalytics]);

  if (!mounted || !weeklyAnalytics) {
