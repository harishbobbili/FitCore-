...
create table if not exists public.progress_photos (
  ...
  angle       text check (angle in ('front','back','side_left','side_right')) default 'front',
...
alter table public.progress_photos add column if not exists angle text check (angle in ('front','back','side_left','side_right')) default 'front';
alter table public.progress_photos add column if not exists weight_kg numeric(5,2);
alter table public.progress_photos add column if not exists notes text;
alter table public.progress_photos add column if not exists ai_analysis jsonb;
...
drop policy if exists "Users manage own photos" on public.progress_photos;
